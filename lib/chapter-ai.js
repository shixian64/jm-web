...
const CHAPTER_AI_PROMPT_VERSION = 'v2';
...
function chapterSourceTitle(chapter) {
  return textValue(chapter?.name) || textValue(chapter?.title);
}
...
function effectiveChapterTitle(sourceName, generatedTitle) {
  return textValue(sourceName) || textValue(generatedTitle);
}
...
class ChapterAiScheduler {
  ...
  config() { return { enabled: !!this.apiKey, model: this.model, promptVersion: CHAPTER_AI_PROMPT_VERSION, intervalMs: this.intervalMs, maxConcurrency: this.maxConcurrency, maxRetries: this.maxRetries, modelTimeoutMs: this.modelTimeoutMs, active: this.active, queued: this.state.queue.length }; }
  ...
  enqueue(aid, photoId, priority = 0) {
    ...
    if (pending) {
      ...
      this.state.queue.push({ key, aid, photoId, priority: Number(priority) || 0, attempts: 0, nextAt: 0 });
    }
  ...
  async analyze(aid, photoId) {
    ...
    const content = [{ type: 'text', text: '请阅读下面完整漫画章节。无论原章节名是否为空，都必须根据图片生成一个新的 chapterTitle；同时必须生成 detailedDescription 和 briefSummary，三个字段必须一起返回且都为字符串。严格只返回 JSON：{ "chapterTitle": string, "detailedDescription": string, "briefSummary": string }。原章节名仅作为参考和默认标题，不得因为已有标题而省略或跳过 AI 标题。briefSummary 是剧情总结，不是标题。不要编造无法从图片确认的内容。原章节名：' + sourceName }];
