...
function addTraceDuration(trace, key, started) {
  ...
  trace[key] = Math.max(0, Number(trace[key]) || 0) + elapsed;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      req = https.request({
        ...
        addTraceDuration(trace, 'ttfb_ms', requestStarted);
        if (settled) {
          incoming.destroy();
          return;
        }
    ...
    req.once('socket', (socket) => {
      const socketStarted = Date.now();
      let connectedAt = socketStarted;
      if (!socket.connecting) {
        // keep-alive 复用连接没有新的 TCP/TLS 握手。
        if (trace) trace.reused_socket = true;
        return;
      }
      socket.once('connect', () => {
        addTraceDuration(trace, 'connect_ms', socketStarted);
        connectedAt = Date.now();
      });
      socket.once('secureConnect', () => addTraceDuration(trace, 'tls_ms', connectedAt));
    });
