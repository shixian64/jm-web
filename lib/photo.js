...
const MAX_CHAPTER_IMAGES = Math.min(
  10000,
  Math.max(1, Number(process.env.JMW_MAX_CHAPTER_IMAGES) || 2000),
);
...
function parsePhotoHtml(html) {
  ...
  if (resultObj && Array.isArray(resultObj.images)) {
    if (resultObj.images.length > MAX_CHAPTER_IMAGES) {
      throw new Error(`章节图片数量超过 ${MAX_CHAPTER_IMAGES} 张上限`);
    }
    for (const item of resultObj.images) {
      // 目前上游通常返回文件名字符串；部分线路会返回带 name/path
      // 字段的对象，兼容它们但不把整段对象原样带到浏览器。
      const name = typeof item === 'string'
        ? item
        : item && typeof item === 'object' && !Array.isArray(item)
          ? (item.name || item.path || item.filename)
          : '';
      if (typeof name !== 'string') continue;
      const cleanName = name.trim();
      if (!cleanName || cleanName.length > 512 || /[\u0000-\u001f\u007f]/.test(cleanName)) continue;
      out.images.push({ name: cleanName });
    }
  }
...
module.exports = { parsePhotoHtml, extractObject, lenientParse, MAX_CHAPTER_IMAGES };
