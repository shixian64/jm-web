...
async function upstreamRequest({
  ...
  dnsLookup, fetchImpl = fetch, signal, onSuccess, retryUnauthorized = false, trace = null,
}) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (trace && typeof trace === 'object') {
      trace.attempts = i + 1;
      trace.retry_count = i;
      try { trace.upstream_host = new URL(host).hostname.toLowerCase().slice(0, 253); } catch (_) { trace.upstream_host = ''; }
      trace.error_type = '';
    }
    ...
    try {
      ...
      if (signal && signal.aborted) throw abortReason(signal);
      if (trace && typeof trace === 'object') {
        trace.upstream_ms = Math.max(0, Date.now() - attemptStarted);
        trace.error_type = 'dns';
      }
    ...
    try {
      ...
      if (trace && typeof trace === 'object') {
        ...
        trace.error_type = /^(?:EAI_|ENOTFOUND|ENODATA)/.test(code) ? 'dns'
          : /TIMEOUT/.test(code) ? 'timeout'
          : /^(?:ECONN|ENET|EHOST|EPIPE|ERR_SOCKET)/.test(code) ? 'connect' : 'network';
      }
      attempt.cleanup();
      if (signal && signal.aborted) throw abortReason(signal);
    ...
    try {
    // 收到响应头后立即记录本次上游耗时；成功响应在读取/解密完成后会
    // 再覆盖为完整耗时。这样 4xx/5xx/重定向等无正文分支不会留下旧值。
    if (trace && typeof trace === 'object') {
      trace.upstream_ms = Math.max(0, Date.now() - attemptStarted);
    }
    ...
    if (res.status >= 300 && res.status < 400) {
      if (trace && typeof trace === 'object') trace.error_type = 'redirect';
    ...
    if (res.status >= 500) {
      if (trace && typeof trace === 'object') trace.error_type = 'http_5xx';
    ...
    if (res.status === 403 && method === 'GET' && i < list.length - 1) {
      if (trace && typeof trace === 'object') trace.error_type = 'http_403';
      lastErr = new ApiError('请求被上游拒绝（HTTP 403）', 403);
      await cancelBody(res);
      continue; // 域名常被逐一封锁 → 尝试下一个
    }
    if (res.status === 401) {
      if (trace && typeof trace === 'object') trace.error_type = 'auth';
    ...
    if ((res.status === 404 || res.status === 410) && method === 'GET' &&
      ...
      if (trace && typeof trace === 'object') trace.error_type = 'http_404';
      lastErr = new ApiError(`上游线路不支持当前接口（HTTP ${res.status}）`, 502);
      await cancelBody(res);
      continue;
    }
    if (res.status >= 400) {
      if (trace && typeof trace === 'object') trace.error_type = 'http_4xx';
      await cancelBody(res);
    ...
    if (trace && typeof trace === 'object') {
      trace.upstream_ms = Math.max(0, Date.now() - attemptStarted);
      trace.error_type = upstreamCode === 401 ? 'auth'
        : upstreamCode >= 500 ? 'http_5xx' : 'upstream_error';
    }
