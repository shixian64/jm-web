...
function setDohState(value) {
  ...
  state = sanitizeState({
    ...state,
    dohEnabled: value?.enabled === undefined ? state.dohEnabled : value.enabled,
    dohAutoStart: value?.autoStart === undefined ? state.dohAutoStart : value.autoStart,
    ...
    dohPreferIpv6: value?.preferIpv6 === undefined ? state.dohPreferIpv6 : value.preferIpv6,
  });
...
async function publicHttpsFetch(input, init = {}, lookup = dns.promises.lookup, dnsTimeoutMs = 8000) {
  ...
  const addresses = await assertPublicUrl(url, lookup, dnsTimeoutMs, signal);
...
async function dohResolve(hostname, providerId = state.dohProvider, timeoutMs = 8000, preferIpv6 = state.dohPreferIpv6, signal) {
  ...
  const cacheKey = `${provider.id}:${provider.url}:${preferIpv6 ? 1 : 0}:${host}`;
  ...
  let v6 = [];
  if (preferIpv6) {
    try { v6 = await resolveType('AAAA', 28); } catch (_) {}
  }
  const v4 = await resolveType('A', 1);
  const addresses = [...new Set([...v6, ...v4])];
  if (!addresses.length) throw new ApiError(`DoH 未返回${preferIpv6 ? ' IPv6/IPv4' : ' IPv4'}地址`, 502);
  pruneDohCache();
...
async function requestAiStream(body, signal) {
  if (!process.env.AI_API_KEY) throw new ApiError('服务器未配置 AI_API_KEY', 503);
  ...
  try {
    response = await outboundFetch(endpoint, {
      ...
      body: JSON.stringify({ model: process.env.AI_MODEL || 'gpt-5-mini', messages, stream: true }),
      redirect: 'error', signal: request.signal,
    });
  } catch (error) {
    request.cleanup();
...
async function searchTavily(text, options, limit, signal) {
  if (!process.env.TAVILY_API_KEY) throw new ApiError('服务器未配置 TAVILY_API_KEY', 503);
...
async function checkUpdate(signal) {
  ...
  try {
    ...
    return { currentVersion, latestVersion, available, message: String(json.name || json.body || '').slice(0, 500), url: String(json.html_url || '') };
  } catch (error) {
    if (signal && signal.aborted) throw abortError(signal);
    return { currentVersion, latestVersion: currentVersion, available: false, message: `更新检查失败：${error.message}`, url: `https://github.com/${repo}/releases` };
  }
}

module.exports = {
  addLog, getLogs, clearLogs,
  getDohState, setDohState, dohLookup, outboundFetch, testDoh,
  aiConfig, requestAiStream, cleanupResponse, searchWeb, checkUpdate,
};
