...
function sanitizeData(raw) {
  ...
  return {
    ...
    extraImageHosts: sanitizeList(raw.extraImageHosts, EXTRA_IMAGE_HOST_LIMIT),
    imgHost: normalizeHost(raw.imgHost),
  };
}
...
function apiHostsForSource(value, jarApiHost) {
  ...
  // builtin/mixed 也不得绕过它回退到内置 origin。
  if (envApiHosts) return [...envApiHosts];
  const source = normalizeDataSource(value);
  const builtins = builtinApiHosts();
  const requested = normalizeHost(jarApiHost) || normalizeHost(data.apiHost);
  const network = [requested && builtins.includes(requested) ? requested : builtins[0]].filter(Boolean);
  if (source === 'builtin') return builtins;
  if (source === 'network') return network;
  return [...new Set([...network, ...builtins])];
}
...
module.exports = {
  ...
  setPreferredImageHost(host) {
    if (host) preferredImageHost = host.replace(/\/+$/, '');
  },
